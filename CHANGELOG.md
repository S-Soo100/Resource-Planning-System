...
- **인라인 카테고리 생성**: CategoryTreeSelect 드롭다운에서 바로 새 카테고리 생성
  - "새 카테고리" 버튼 → 카테고리명 입력 + 부모 선택 → 즉시 생성 + 자동 선택
  - 엑셀 벌크 업로드 미리보기에서도 카테고리 실시간 생성 가능

- **품목 일괄 등록 양식 다운로드**: 엑셀 업로드 페이지에서 샘플 양식(.xlsx) 다운로드

- **공용 Toggle 컴포넌트**: 접근성 완비 토글 스위치
  - `role="switch"`, `aria-label`, `aria-checked` 속성
  - 44px 터치 타겟 확보 (모바일 접근성)
  - green/blue 색상 옵션

- **v2.8.1 E2E 테스트**: 23건 신규 테스트 추가 (카테고리, 품목 UX, 시리얼코드, 엑셀, 인라인 생성 등)

### 수정됨 (Fixed)

- **서비스 품목 가격 데이터 누수**: `isService=true` 시 가격 데이터가 서버에 그대로 전송되던 버그 수정 → DTO에서 클리어 후 전송
- **costPrice 0원 입력 무시**: `parseInt` falsy 체크로 0원이 undefined로 씹히던 버그 수정
- **categoryId 검증 누락**: UI에 필수 표시(`*`)가 있으나 실제 폼 검증에서 빠져있던 이슈 수정
- **buildCategoryTree 3단계+ 트리**: 중간 노드 누락 버그 수정 (재귀 체크)
- **useEffect deps 누락**: 팀 전환 시 카테고리 펼침 상태 미초기화 수정

### 개선됨 (Improved)

- **TeamItemModal 권한 검증 강화**: `usePermission()` 훅 추가, `canViewCostPrice`/`canEditPrice`로 원가 필드 접근 제어
- **카테고리 CRUD 캐시 무효화**: `alert` → `toast` 전환, categoryTree 캐시 즉시 갱신
- **품목 선택 모달**: 문자열 → ID 기반 카테고리 필터, 트리 구조 계층형 UI
- **서비스 품목 토글**: 상단 이동, 가격/건보 필드 조건부 렌더링
- **시리얼코드**: 건보 시리얼은 건강보험 등록 품목에만 표시
- **엑셀 일괄 등록**: 행 삭제 기능, 빈 품목명 행 자동 제외
- **판매 데이터 원가**: averageCost 우선 → costPrice 폴백 계산
- **용어 통일**: "아이템" → "품목", 서비스 배지 green 통일
- **"가격 정보" 텍스트 충돌 해소**: 안내 문구와 섹션 헤더 텍스트 분리
...
- **판매 내역 테이블 2단 행 UI 전면 개선** 🎨
  - 기존 12개 컬럼(마진 권한 시) → 5개 핵심 컬럼으로 대폭 축소
    - 헤더: 판매일자 | 판매처 | 제목 | 판매가/마진 | 명세서
  - **제목 셀 아래에 부가 정보 배치**: 담당자, 품목 수, 입금상태를 제목 바로 아래에 작은 글씨로 표시
  - **판매가 셀 아래에 원가/마진 배치**: 판매가 바로 아래에 원가, 마진액(마진율)을 한눈에 확인 가능 (마진 권한자만)
  - 판매처명 6글자 초과 시 말줄임(...) 처리 + hover 시 전체 이름 tooltip 표시
  - 마진 정보 줄바꿈 방지 (whitespace-nowrap)
  - 합계 행도 동일한 2단 구조로 통일 (총 판매가 아래에 총 원가/마진/평균 마진율)

- **세금계산서 섹션 UI 개선** (E-012)
  - 세금계산서 섹션을 거래 정보에서 분리하여 독립 섹션으로 배치

- **발주 상세 거래 정보 섹션 UX 향상** (E-011)

- **고객 추가 모달 재설계**
  - 고객 유형(B2C/B2B) 버튼형 선택을 최상단에 배치

- **고객 상세 페이지 UX 전면 개선** (E-009, E-010)

- **판매 내역 미수금 집계 + 입금액 컬럼 추가** (E-001, E-003, E-004)
  - 미수금 건수/금액 요약, 입금액 컬럼, 입금 상태 필터

### 수정됨 (Fixed)

- **재구매 예정 고객 메뉴** 고객관리 탭으로 이동 + API 응답 방어 처리
- **suppliers undefined 방어 코드** 일괄 적용 (B-001~B-003)
- **DetailsEditModal** suppliers 방어 코드 누락 수정 (E-007)

### 리팩토링 (Refactored)

- User → Supplier 마이그레이션 레거시 코드 정리 (E-008)
...
- **고객관리 시스템 (v2.5)** 🎉
  - 재구매 예정 고객 목록 페이지 (`/repurchase`): 구매 후 경과일 기반 고객 관리
  - 메인 대시보드 상단에 재구매 예정 고객 알림 위젯 (N명 표시, 클릭 시 이동)
  - B2C/B2B/미분류 필터 탭 + 필터 적용 엑셀 내보내기
  - 경과일별 4단계 요약 카드 (3일 이내 / 3~7일 / 7~13일 / 14일 이상)

- **고객 서류 관리**
  - 발주 상세 페이지에 고객 서류 업로드/조회/삭제 기능
  - 팀 멤버 관리(UserEditModal)에서도 고객 서류 접근 가능
  - 서류 유형별 분류: 수급자증명서, 진단서, 처방전, 기타

- **세금계산서 관리**
  - 발주 상세 페이지에 세금계산서 섹션 추가
  - 파일 업로드 + 목록 + 삭제 기능

- **출고완료 건 수정 기능 확장**
  - 환급 상태, 입금 상태, 세금계산서 필드 추가
  - 환급 해당없음 시 관련 필드 자동 비활성화
  - 입금 상태 Select 드롭다운 (자부담금/전액/선금/중도금/잔금)

- **고객 정보 필드 확장**
  - UserEditModal에 주민등록번호, 수급자 여부, 고객 유형(B2C/B2B) 필드 추가
  - 주민등록번호 마스킹: 읽기 전용 모드에서 `900101-*******` 형태로 표시

- **E2E 테스트**: 고객관리 기능 전체 Playwright 테스트 추가 (20개 시나리오)

### 수정됨 (Fixed)

- **API 응답 double-wrapping 수정**: 백엔드가 이미 `{ success, data }` 래핑한 응답을 프론트에서 중복 래핑하던 버그 수정
  - 영향 범위: 재구매 예정 고객, 고객 서류, 세금계산서 API
- **DetailsEditModal 방어 코드 추가**: suppliers가 배열이 아닐 때 발생하는 TypeError 수정
- **API 403 에러 처리 개선**: 로그아웃 대신 경고만 표시하도록 변경
- **styled-jsx 타입 선언 추가**: CI 빌드 시 `<style jsx>` TypeScript 에러 수정

### 개선됨 (Improved)

- **팀 권한 시스템 전면 전환** (v3.0)
  - 전체 페이지를 `usePermission()` 훅 기반 팀 권한으로 마이그레이션 완료
  - Admin, 기준정보, 재고, 판매/거래처, 발주, 시연, 메뉴 페이지 모두 전환
  - `User.accessLevel` 직접 참조 제거 → `TeamUserMapping.accessLevel` 단일 소스

### 문서 (Documentation)

- CLAUDE.md 권한 마이그레이션 완료 상태로 업데이트
- 고객관리 기획 결정 사항 문서 추가 (`docs/2.5/DECISIONS.md`)
...
- **가격 입력 UX 대폭 개선** ⚡
  ...
  - **입력 시간 50% 단축** (4회 입력 → 2회 입력)

- **발주 요청 품목 테이블 반응형 최적화** 📱
  - 재고 컬럼 제거 → 품목명 하위에 색상 구분 통합 표시 (초록/빨강)
  - 컬럼 수 감소: 9개 → 8개
  - 모든 패딩 축소: `px-4` → `px-2` (테이블 전체)
  - 아이콘/버튼 크기 감소: 수량 버튼, 체크박스, 삭제 버튼
  - 반응형 컬럼 숨김:
    - 모바일 (< 768px): 영세율 체크박스 숨김 → 핵심 4개 컬럼만 표시
    - 태블릿 (< 1024px): 공급가액/부가세 숨김 → 6개 컬럼 표시
    - 작은 모바일 (< 640px): 메모 입력 숨김
  - 헤더 라벨 간소화: "✏️입력" → "✏️", "💡자동" → "💡"
  - 모바일에서 가로 스크롤 최소화로 UX 향상
