...
- **해결일**: 2026-03-03
- **적용 방안**: 방안 1 (router.prefetch) + 방안 2 (15초 타임아웃 + 새로고침)
...
- [x] 팀 선택 후 로딩 화면에 타임아웃 처리 추가 (15초 초과 시 새로고침 안내)
- [ ] ~~또는: 로딩 중 프로그레스 바/단계 표시 (API 호출 → 데이터 로드 → 페이지 전환)~~ (불필요)
- [x] 또는: 팀 선택 로직 최적화 (router.prefetch로 /menu 페이지 사전 컴파일)
