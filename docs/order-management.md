...
#### 고객(Supplier) 필수 정책 (v2.3+)

> **용어 변경**: v2.3부터 "거래처" → "고객사" → "고객"으로 명칭 변경 (UI 일관성 향상)
...
**동작:**
1. **필수 선택**: 발주 생성 시 고객을 반드시 선택해야 함
2. **자동 입력**: 고객 선택 시 수령인 정보(이름, 연락처, 주소) 자동 입력
3. **수정 가능**: 자동 입력된 수령인 정보는 이후 자유롭게 수정 가능
   - 예: "OO의료기기" 선택 후 → 배송지를 "OO의료기기 대구지점"으로 변경 가능
4. **필터링 용도**: `supplierId`는 주로 발주 기록 필터링 및 고객별 조회에 사용
5. **모든 계정 적용**: Supplier 계정(납품처)도 고객 선택/생성 가능 (v2.3.1)

**UI/UX 개선 (v2.3.1):**

##### 모달 기반 고객 선택
기존 드롭다운 방식에서 모달 기반 선택으로 전면 개편:

- **SelectSupplierModal**: 고객 검색 및 선택 전용 모달
  - 검색 기능: 고객명, 전화번호, 주소로 실시간 검색
  - 카드 형태 레이아웃: 고객 정보 미리보기
  - 즉시 선택: 고객 클릭 시 바로 선택 및 모달 자동 닫기 (확인 버튼 없음)
  - 테마 색상: 일반 발주(파란색) / 휠체어 발주(보라색)

##### 고객 등록 접근성
- **모달 내 등록**: SelectSupplierModal 상단에 "새 고객 등록" 버튼 배치
- **z-index 관리**: AddSupplierModal (z-60) > SelectSupplierModal (z-50)
- **즉시 반영**: 고객 등록 후 React Query 캐시 무효화로 자동 UI 업데이트
- **폼 데이터 보존**: 페이지 새로고침 없이 등록된 고객 즉시 선택 가능

##### 검증 강화
- **UI 레이어**: 폼 제출 전 supplierId 필수 검증
- **서비스 레이어**: orderService.createOrderDto에서 명시적 검증
  ```typescript
  if (!formData.supplierId) {
    throw new Error("고객을 선택해주세요");
  }
  ```
- **타입 안전성**: CreateOrderDto.supplierId는 `number` (nullable 불가)

##### 자동 채우기 정책
- 고객 선택 시 다음 필드 자동 입력:
  - 수령인: `representativeName` 또는 `supplierName`
  - 수령인 연락처: `supplierPhoneNumber`
  - 수령지 주소: `supplierAddress`
  - 상세 주소: 빈 값 (사용자가 직접 입력)
- 자동 입력 후 **수동 수정 가능** (배송지 변경 등)

##### 컴포넌트 구조
```typescript
// 고객 선택 모달
SelectSupplierModal.tsx
├── Props: suppliers, onSelect, onAddSupplier, focusRingColor
├── 검색 기능: useMemo로 필터링 최적화
├── 카드 레이아웃: 고객명, 전화번호, 주소, 대표자명 표시
└── 즉시 선택: handleSelect → onSelect + onClose

// 발주 폼 통합
OrderRequestForm.tsx / WheelchairOrderForm.tsx
├── SelectSupplierModal 상태 관리
├── handleSupplierSelect: 자동 채우기 로직
└── AddSupplierModal 통합 (고객 등록)

// 공통 섹션 컴포넌트
SupplierSection.tsx
├── 고객 선택 전: "고객 선택하기" 버튼 (모달 열기)
├── 고객 선택 후: 선택된 고객 정보 표시 + "변경" 버튼
└── 배송지 변경 안내 메시지
```
