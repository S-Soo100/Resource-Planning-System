...
```
1. 사용자 발주 요청 (requested) ← 수정/삭제 가능
   ↓
2. 1차승인권자 검토
   ├─ 승인 (approved) ← 수정/삭제 불가
   └─ 반려 (rejected) ← 수정/삭제 불가
   ↓
3. 관리자 출고 처리
...
1. **권한**: 작성자 본인 또는 Admin 권한
2. **상태**: `requested` (승인 이전) 상태만 삭제 가능

#### 삭제 권한 매트릭스

| 사용자 타입     | 본인 발주 (requested) | 타인 발주 (requested) | 승인된 발주  |
| --------------- | --------------------- | --------------------- | ------------ |
| **일반 사용자** | ✅ 삭제 가능          | ❌ 삭제 불가          | ❌ 삭제 불가 |
| **Admin**       | ✅ 삭제 가능          | ✅ 삭제 가능          | ❌ 삭제 불가 |

- **삭제 확인**: 발주자, 수령자, 상태 정보와 함께 확인 다이얼로그 표시
- **실시간 반영**: 삭제 후 발주 목록 자동 새로고침
