...
function TeamItemsContent() {
  ...
  useEffect(() => {
    ...
    if (editId && teamItems.length > 0) {
      const itemToEdit = teamItems.find(
        (item) => item.id === parseInt(editId, 10)
      );
      if (itemToEdit) {
  ...
  if (isLoading || isUserLoading || isCategoryLoading || isPermissionLoading) {
    return (
      <div className="p-4">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {Array.from({ length: 4 }).map((_, i) => (
                <div
                  key={i}
                  className="p-4 h-24 bg-white rounded-xl shadow-sm"
                ></div>
              ))}
            </div>
          </div>
  ...
  const isReadOnly = permissionReadOnly;
