...
const EventItem: React.FC<EventItemProps> = ({
  ...
  const { getEventColor, getEventStatusText, getEventSummary } =
    useCalendarEvents([event]);
      ...
      >
        <div className="flex items-center gap-1">
          ...
          <span className="font-medium truncate">
            {isMobile && `[${getEventTypeText()}] `}
            {getDisplayTitle()}
          </span>
        </div>
        <div className="text-xs opacity-75 truncate">{statusText}</div>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-2">
          <div className={`${colors.text}`}>{getEventIcon()}</div>
          <div>
            ...
            <span className="text-xs text-gray-500">
              ...
              {/* 시연 기간 정보 표시 */}
              {event.type === "demo" &&
                (event.details as DemoEventDetails).spanInfo &&
                (event.details as DemoEventDetails).spanInfo!.totalDays > 1 && (
                  <span className="ml-2 px-1 bg-purple-100 text-purple-700 rounded text-xs">
                    {(event.details as DemoEventDetails).spanInfo!.dayIndex + 1}
                    /{(event.details as DemoEventDetails).spanInfo!.totalDays}
                    일차
                  </span>
                )}
            </span>
      ...
      {/* 호버 효과 */}
      {isHovered && (
        <div className="mt-2 text-xs text-gray-500">클릭하여 자세히 보기</div>
      )}
    </div>
  );
};
  ...
  return (
    <div
      className={`
        ...
        ${className}
      `}
      title={`${event.type === "order" ? "판매" : "시연"}: ${event.title}`}
    />
  );
};
