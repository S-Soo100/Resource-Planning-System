...
const MonthView: React.FC<MonthViewProps> = ({
  ...
  const weekMaxLayers = useMemo(() => {
    ...
    demoEvents.forEach((demo) => {
      const layerIndex =
        demoLayerMap.get((demo.details as DemoEventDetails).id) || 0;
      ...
      monthInfo.weeks.forEach((week, weekIndex) => {
        week.forEach((date) => {
          if (formatDateToString(date) === dateStr) {
            layersPerWeek[weekIndex] = Math.max(
              layersPerWeek[weekIndex],
              layerIndex + 1
            );
          }
        });
    ...
    <div className={`bg-white rounded-lg shadow-md ${className}`}>
      ...
      <div className="relative">
          ...
          return (
            ...
            >
              {week.map((date, dayIndex) => {
                ...
                const dayOrderEvents = dayEvents.filter(
                  (e) => e.type === "order"
                );
                ...
                `}
                    ...
                    {dayDemoEvents.map((demo) => {
                      ...
                      const spanInfo = (demo.details as DemoEventDetails)
                        .spanInfo;
                    ...
                    {dayDemoEvents.map((demo) => {
                      const demoId = (demo.details as DemoEventDetails).id;
                      const layerIndex = demoLayerMap.get(demoId) || 0;
                      const eventSpanInfo = (demo.details as DemoEventDetails)
                        .eventSpanInfo;
                    ...
                    {hasEvents && (
                      <div className="absolute bottom-1 left-1 right-1 flex justify-center gap-1">
                        {dayEvents.slice(0, 4).map((event) => (
                          <EventDot key={event.id} event={event} size="xs" />
                        ))}
                        {dayEvents.length > 4 && (
        ...
        {demoEvents.map((demo) => {
          ...
          // 시작일 또는 월요일에만 제목 표시
          if (
            shouldShowTitle(spanInfo, columnIndex) &&
            columnIndex !== -1 &&
            rowIndex !== -1
          ) {
            // 이전 주들의 높이 합계 계산
            ...
            return (
              ...
              >
                ...
                >
                  물품:{" "}
                  {formatShortDate(
                    (demo.details as DemoEventDetails).demoStartDate
                  )}
                  ~
                  {formatShortDate(
                    (demo.details as DemoEventDetails).demoEndDate
                  )}
                  {(demo.details as DemoEventDetails).eventStartDate &&
                    ` · 행사: ${formatEventSchedule(
                      (demo.details as DemoEventDetails).eventStartDate,
      ...
      {selectedDate && (
        <div className="border-t border-gray-200 bg-gray-50 p-4">
          <h4 className="font-semibold text-gray-700 mb-3">
            ...
            {selectedDate.getDate()}일{getDayName(selectedDate.getDay())}요일
            일정
          </h4>
