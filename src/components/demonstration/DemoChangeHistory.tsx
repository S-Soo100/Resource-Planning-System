...
import { useDemoChangeHistory } from '@/hooks/useChangeHistory';
...
const DemoChangeHistory: React.FC<DemoChangeHistoryProps> = ({ demoId }) => {
  ...
  const { data, isLoading, isError, error } = useDemoChangeHistory(
    demoId,
    {
      page,
      limit: 10,
  ...
  // SSE 실시간 알림 (임시 비활성화)
  // const { isConnected } = useChangeHistorySSE('demo', demoId, isExpanded);
        ...
        <div className="flex items-center gap-2">
          ...
          {isExpanded && data && data.data.length > 0 && (
            <Button
              ...
              onClick={() => exportChangeHistory(data.data, 'demo', demoId)}
            >
              엑셀 내보내기
            </Button>
          )}
...
export default DemoChangeHistory;
