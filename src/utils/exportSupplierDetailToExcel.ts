...
/**
 * 판매대상 상세 데이터를 엑셀로 내보내기
 * @param supplier 판매대상 정보
...
export const exportSupplierDetailToExcel = (
  ...
  const supplierInfoData = [
    ...
    [
      "조회 기간",
      dateRange ? `${dateRange.startDate} ~ ${dateRange.endDate}` : "전체",
    ],
  ];

  const ws1 = XLSX.utils.aoa_to_sheet(supplierInfoData);
  XLSX.utils.book_append_sheet(wb, ws1, "판매대상 정보");
  ...
  if (showMarginColumns) {
    ...
    const recordsWithMargin = salesRecords.filter(
  ...
  // 파일 다운로드
  const defaultFilename = `판매대상_${supplier.supplierName}_${format(
    new Date(),
    "yyyyMMdd_HHmmss"
  )}.xlsx`;
